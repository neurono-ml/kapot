...
use kapot::{extension::SessionConfigExt, prelude::SessionContextExt};
use kapot_cli::{
...
pub async fn main() -> Result<()> {
    ...
    let mut kapot_config =
        SessionConfig::new_with_kapot().with_information_schema(true);

    if let Some(batch_size) = args.batch_size {
        kapot_config = kapot_config.with_batch_size(batch_size);
    };
    ...
    let ctx = match (args.host, args.port) {
        ...
        _ => {
            if let Some(concurrent_tasks) = args.concurrent_tasks {
                kapot_config =
                    kapot_config.with_target_partitions(concurrent_tasks);
            };
            let state = SessionStateBuilder::new()
                .with_config(kapot_config)
                .with_default_features()
