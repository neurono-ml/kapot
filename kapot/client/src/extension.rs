...
use kapot_core::extension::SessionConfigHelperExt;
pub use kapot_core::extension::{SessionConfigExt, SessionStateExt};
use kapot_core::{
...
///
/// Provided methods set up [SessionContext] with [kapotQueryPlanner](kapot_core::utils), which
/// handles running plans on kapot clusters.
///
///```no_run
/// use kapot::prelude::SessionContextExt;
...
pub trait SessionContextExt {
    /// Creates a context for executing queries against a standalone kapot scheduler instance
    ///
    /// It wills start local kapot cluster with scheduler and executor.
    ...
    /// Creates a context for executing queries against a remote kapot scheduler instance
...
impl SessionContextExt for SessionContext {
    async fn remote_with_state(
        ...
        let session_state =
            state.upgrade_for_kapot(scheduler_url, remote_session_id)?;
    ...
    async fn remote(url: &str) -> datafusion::error::Result<SessionContext> {
        ...
        let session_state =
            SessionState::new_kapot_state(scheduler_url, remote_session_id)?;
    ...
    async fn standalone_with_state(
        ...
        let session_state =
            state.upgrade_for_kapot(scheduler_url, remote_session_id.clone())?;
    ...
    async fn standalone() -> datafusion::error::Result<Self> {
        ...
        let session_state =
            SessionState::new_kapot_state(scheduler_url, remote_session_id.clone())?;
...
impl Extension {
    ...
    async fn setup_standalone(
        ...
        use kapot_core::{serde::KapotCodec, utils::default_config_producer};

        let addr = match session_state {
            None => kapot_scheduler::standalone::new_standalone_scheduler()
                .await
                .map_err(|e| DataFusionError::Configuration(e.to_string()))?,
            Some(session_state) => {
                kapot_scheduler::standalone::new_standalone_scheduler_from_state(
                    session_state,
                )
                .await
                .map_err(|e| DataFusionError::Configuration(e.to_string()))?
        ...
        let concurrent_tasks = config.kapot_standalone_parallelism();
        ...
        match session_state {
            ...
            Some(session_state) => {
                kapot_executor::new_standalone_executor_from_state(
                    scheduler,
                    concurrent_tasks,
                    session_state,
                )
    ...
    async fn setup_remote(
        ...
        let limit = config.kapot_grpc_client_max_message_size();
