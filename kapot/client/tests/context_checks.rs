...
mod supported {
    ...
    #[case::remote(remote_context())]
    #[tokio::test]
    async fn should_execute_sql_show_configs_kapot(
        #[future(awt)]
        #[case]
        ctx: SessionContext,
    ) -> datafusion::error::Result<()> {
        let state = ctx.state();
        let kapot_config_extension =
            state.config().options().extensions.get::<KapotConfig>();

        // kapot configuration should be registered with
        // session state
        assert!(kapot_config_extension.is_some());

        let result = ctx
            .sql("select name, value from information_schema.df_settings where name like 'kapot.%' order by name limit 2")
            .await?
            .collect()
            .await?;
        ...
        let expected = [
            ...
            "| name                                  | value    |",
            "+---------------------------------------+----------+",
            "| kapot.grpc_client_max_message_size | 16777216 |",
            "| kapot.job.name                     |          |",
            "+---------------------------------------+----------+",
        ];
    ...
    async fn should_execute_sql_set_configs(
        ...
        ctx.sql("SET kapot.job.name = 'Super Cool kapot App'")
            .await?
            .show()
            .await?;

        let result = ctx
            .sql("select name, value from information_schema.df_settings where name like 'kapot.job.name' order by name limit 1")
            .await?
            .collect()
            .await?;
