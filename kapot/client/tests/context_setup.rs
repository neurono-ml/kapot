...
mod standalone {
    ...
    use kapot_core::serde::KapotPhysicalExtensionCodec;
    use datafusion::{
        assert_batches_eq,
        common::exec_err,
    ...
    async fn should_set_logical_codec() -> datafusion::error::Result<()> {
        ...
        let session_config = SessionConfig::new_with_kapot()
            .with_information_schema(true)
            .with_kapot_logical_extension_codec(codec.clone());
    ...
    async fn should_set_physical_codec() -> datafusion::error::Result<()> {
        ...
        let session_config = SessionConfig::new_with_kapot()
            .with_information_schema(true)
            .with_kapot_physical_extension_codec(physical_codec.clone());
    ...
    async fn should_override_planner() -> datafusion::error::Result<()> {
        let session_config = SessionConfig::new_with_kapot()
            .with_information_schema(true)
            .with_kapot_query_planner(Arc::new(BadPlanner::default()));
        ...
        let session_config =
            SessionConfig::new_with_kapot().with_information_schema(true);
    ...
    impl Default for MockPhysicalCodec {
        fn default() -> Self {
            Self {
                ...
                codec: Arc::new(KapotPhysicalExtensionCodec::default()),
            }
        }
    }
