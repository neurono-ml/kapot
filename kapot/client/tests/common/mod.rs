...
use kapot::prelude::{SessionConfigExt, SessionContextExt};
use kapot_core::serde::{
    protobuf::scheduler_grpc_client::SchedulerGrpcClient, KapotCodec,
};
use kapot_core::{ConfigProducer, RuntimeProducer};
use kapot_scheduler::SessionBuilder;
...
/// starts a kapot cluster for integration tests
#[allow(dead_code)]
pub async fn setup_test_cluster() -> (String, u16) {
    let config = SessionConfig::new_with_kapot();
    let default_codec = KapotCodec::default();

    let addr = kapot_scheduler::standalone::new_standalone_scheduler()
        .await
    ...
    kapot_executor::new_standalone_executor(
        scheduler,
        config.kapot_standalone_parallelism(),
        default_codec,
    )
    .await
...
pub async fn setup_test_cluster_with_state(session_state: SessionState) -> (String, u16) {
    ...
    let addr = kapot_scheduler::standalone::new_standalone_scheduler_from_state(
        &session_state,
    )
    .await
    .expect("scheduler to be created");
...
pub async fn setup_test_cluster_with_builders(
    ...
    let logical = config.kapot_logical_extension_codec();
    let physical = config.kapot_physical_extension_codec();
    let codec: KapotCodec<
        datafusion_proto::protobuf::LogicalPlanNode,
        datafusion_proto::protobuf::PhysicalPlanNode,
    > = KapotCodec::new(logical, physical);

    let addr = kapot_scheduler::standalone::new_standalone_scheduler_with_builder(
        session_builder,
    ...
    kapot_executor::new_standalone_executor_from_builder(
        scheduler,
        config.kapot_standalone_parallelism(),
        config_producer,
        runtime_producer,
...
fn init() {
    ...
    let _ = env_logger::builder()
        ...
        .parse_filters(
            "kapot=debug,kapot_scheduler-rs=debug,kapot_executor=debug",
        )
        //.parse_filters("kapot=debug,kapot_scheduler-rs=debug,kapot_executor=debug,datafusion=debug")
        .is_test(true)
        .try_init();
}
