...
//
// The tests are extracted from context.rs where `kapotContext` lives.
// to be checked if `SessionContextExt` has same functionality like `kapotContext`
//
#[cfg(test)]
...
mod basic {
    ...
    async fn test_kapot_show_tables() {
        ...
        // used to fail with kapot context
        // assert!(df.is_err());
        assert!(df.is_ok());
