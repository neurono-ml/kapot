...
use crate::serde::KapotLogicalExtensionCodec;
...
/// [kapotQueryPlanner] planner takes logical plan
...
impl<T: AsLogicalPlan> std::fmt::Debug for KapotQueryPlanner<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("kapotQueryPlanner")
...
impl<T: 'static + AsLogicalPlan> KapotQueryPlanner<T> {
    pub fn new(scheduler_url: String, config: KapotConfig) -> Self {
        Self {
            scheduler_url,
            config,
            extension_codec: Arc::new(KapotLogicalExtensionCodec::default()),
...
mod test {
    ...
    async fn should_detect_select_from_information_schema_as_local_plan() -> Result<()> {
        let ctx = context();
        let df = ctx.sql("SELECT * FROM information_schema.df_settings WHERE NAME LIKE 'kapot%'").await?;
        let lp = df.logical_plan();
        let mut local_run = LocalRun::default();
