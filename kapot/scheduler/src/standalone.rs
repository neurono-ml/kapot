...
pub async fn new_standalone_scheduler_from_state(
    ...
    let codec = KapotCodec::new(logical, physical);
...
pub async fn new_standalone_scheduler_with_builder(
    ...
    let cluster =
        KapotCluster::new_memory("localhost:50050", session_builder, config_producer);
    ...
    let server = SchedulerGrpcServer::new(scheduler_server.clone())
        .max_decoding_message_size(config.kapot_grpc_client_max_message_size())
        .max_encoding_message_size(config.kapot_grpc_client_max_message_size());
