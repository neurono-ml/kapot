...
use kapot_core::config::{LogRotationPolicy, TaskSchedulingPolicy};
...
use kapot_core::serde::{
    KapotCodec, KapotLogicalExtensionCodec, KapotPhysicalExtensionCodec,
};
use kapot_core::utils::{
    create_grpc_client_connection, create_grpc_server, default_config_producer,
    get_time_before,
};
use kapot_core::{ConfigProducer, RuntimeProducer, KAPOT_VERSION};
...
pub struct ExecutorProcessConfig {
    ...
    pub override_function_registry: Option<Arc<KapotFunctionRegistry>>,
...
pub async fn start_executor_process(
    ...
    info!(
        "Executor starting ... (Datafusion kapot {})",
        KAPOT_VERSION
    );
    ...
    let connection = if connect_timeout == 0 {
        create_grpc_client_connection(scheduler_url)
            .await
            .map_err(|_| {
                KapotError::GrpcConnectionError(
...
async fn flight_server_task(
    ...
    tokio::spawn(async move {
        ...
        let server_future = create_grpc_server()
            .add_service(
                FlightServiceServer::new(KapotFlightService::new())
...
async fn clean_all_shuffle_data(work_dir: &str) -> kapot_core::error::Result<()> {
    let mut dir = fs::read_dir(work_dir).await?;
    let mut to_deleted = Vec::new();
    while let Some(child) = dir.next_entry().await? {
        if let Ok(metadata) = child.metadata().await {
            ...
            override_arrow_flight_service: Some(std::sync::Arc::new(
                move |address, mut grpc_shutdown| {
                    tokio::spawn(async move {
                        ...
                        let server_future = kapot_core::utils::create_grpc_server()
                        .add_service(
                            arrow_flight::flight_service_server::FlightServiceServer::new(
                                crate::flight_service::KapotFlightService::new(),
                            ),
                        )
                        .serve_with_shutdown(address, grpc_shutdown.recv());
                        ...
                        server_future.await.map_err(|e| {
                            ...
                            kapot_core::error::KapotError::TonicError(e)
                        })
                    })
                },
            )),
