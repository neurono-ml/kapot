...
docker build -t kapot-builder --build-arg EXT_UID="$(id -u)" -f dev/docker/kapot-builder.Dockerfile .

# run cargo & yarn builds inside the builder container
docker run -v $(pwd):/home/builder/workspace --env RELEASE_FLAG=$RELEASE_FLAG kapot-builder
